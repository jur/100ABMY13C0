...
#define FOR_DUAL_BAND
#endif
#define WSCD_VAP_INTERFACE		  "wlan0-va0"
#define WSCD_WLAN2_INTERFACE	  "wlan2"
#endif
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT	
#define GET_CURRENT_ADDRESS ((pCtx->InterFaceComeIn == COME_FROM_WLAN2)?pCtx->our_addr3:((pCtx->InterFaceComeIn == COME_FROM_WLAN1)?pCtx->our_addr2:pCtx->our_addr))
#else
#ifdef FOR_DUAL_BAND
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
#define GET_CURRENT_INTERFACE ((pCtx->InterFaceComeIn == COME_FROM_WLAN2)?pCtx->wlan_interface_name3:((pCtx->InterFaceComeIn == COME_FROM_WLAN1)?pCtx->wlan_interface_name2:pCtx->wlan_interface_name))
#else
#ifdef FOR_DUAL_BAND
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
#define GET_CURRENT_SOCKET ((pCtx->InterFaceComeIn == COME_FROM_WLAN2)?pCtx->socket3:((pCtx->InterFaceComeIn == COME_FROM_WLAN1)?pCtx->socket2:pCtx->socket))
#else
#ifdef FOR_DUAL_BAND
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
#define WSCD_IND_ONLY_INTERFACE2  ("/var/wps_start_interface2")
#endif
...
typedef struct context 
{
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
	int wlan0_va0_wsc_disabled; /* FOR_THREE_BAND  wlan0-va0 or wlan2 */
#endif
#ifdef FOR_DUAL_BAND
	int socket2;
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
	unsigned char inter2only;
	char wlan_interface_name3[40];
	char fifo_name3[50];
	int socket3;
	unsigned char our_addr3[ETHER_ADDRLEN];
	char SSID3[WSC_MAX_SSID_LEN+1];
	int mixedmode3;
	int auth_type3;
	int encrypt_type3;
	char network_key3[MAX_NETWORK_KEY_LEN+1];
	unsigned char wep_key23[MAX_NETWORK_KEY_LEN+1];
	unsigned char wep_key33[MAX_NETWORK_KEY_LEN+1];
	unsigned char wep_key43[MAX_NETWORK_KEY_LEN+1]; 
	unsigned char wep_transmit_key3;
	int network_key_len3;
	int auth_type_flash3;
	int encrypt_type_flash3;
	int fifo3;
#endif
#endif	
	...
	int role;
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
	int three_wps_icons;        // got from flash mib
	int three_wps_icons_param;  // got from console input
#endif
	int original_role; 
	int use_ie;
	int config_state;
...
#ifdef CONFIG_RTL_TRI_BAND_SUPPORT
extern int write_param_to_flash3(CTX_Tp pCtx, int is_local);
#endif
