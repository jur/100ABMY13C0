...
unsigned char tempbuf2[256];

static int issue_bssTransReq(char *ifname, unsigned char * macaddr, int chanUtil, unsigned char * bssid_mac, int channel)
{
    ...
    memcpy(bssTrans_req.bssid_mac, bssid_mac, MACADDRLEN);
    bssTrans_req.channel = channel;
...
static void do_bss_transition(char *ifname, char * macaddr_str, int chanUtil, char * bssid_str, int channel)
{
    int i;
    ...
    for(i=0; i<MACADDRLEN; i++)
    {
        ...
        tempbuf2[0] = bssid_str[2*i];

        tempbuf[1] = macaddr_str[2*i+1];
        tempbuf2[1] = bssid_str[2*i+1];
        
        tempbuf[2] = 0;
        tempbuf2[2] = 0;
        
        macaddr[i] = strtol((char *)tempbuf, NULL, 16);
        bssid[i] = strtol((char *)tempbuf2, NULL, 16);
    }
	
    if(issue_bssTransReq(ifname, macaddr, chanUtil, bssid, channel))
...
void usage(char *name)
{
    ...
    printf("%s [-c command] [-i interface_name] [-a mac_addr] [-b target_bssid] [-n channel]\n", name);
    printf("\nAvailable commands:\n");
...
{
    cmd_type_t cmd_id;
    char *command = NULL, *mac_str = NULL, *bssid_str = NULL;
    char * interface_name = NULL;
    int c;
    int chanUtil = 0, channel = 0;

    /* Parse options */
    while ((c = getopt (argc, argv, "hc:a:i:p:b:n:")) != -1)
    {
        switch (c)
        {
            case 'c':
    ...
    {
    	case BSS_TRANS_CMD:
		do_bss_transition(interface_name, mac_str, chanUtil, bssid_str, channel);	
		break;
        default:
            break;
    }
