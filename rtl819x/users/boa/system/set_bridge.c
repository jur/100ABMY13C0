...
/*static dhcp entry static_lease 000102030405 192.168.1.199*/
	...
	int count=1;
	char buf[64]={0};
	pf=fopen("/var/curr_br0_info","r");
	if ( NULL != pf ){
		while(fgets(buf,64,pf) != NULL) {
			if(count == 1){
				if(strcmp(buf,"") == 0) {
					system("rm /var/curr_br0_info");
					break;
				}
...
void set_lan_dhcpc(char *iface)
{
	...
	/*They will support ZYXEL solution, so add them.*/
	FILE *pf=NULL;
	pf=fopen("/config/uci/network","r");
	if ( NULL != pf ){
		printf("============== Support ZYXEL solution. ============== \n");
		system("echo $(uci get network.br0.ipaddr) > /var/curr_br0_info");
		system("echo $(uci get network.br0.netmask) >> /var/curr_br0_info");
		system("echo $(uci get network.br0.gateway) >> /var/curr_br0_info");

		FILE *pf2=NULL;
		int count=1;
		char buf[64]={0};
		pf2=fopen("/var/curr_br0_info","r");
		if ( NULL != pf2 ){
			while(fgets(buf,64,pf2) != NULL) {
				if(count == 1){
					if(strcmp(buf,"") == 0) {
						stpcpy(buf,"0.0.0.0");
					}
	...
	//system(cmdBuff); /*They will support ZYXEL solution, so mark it.*/

	/*They will support ZYXEL solution, so add them.*/
	RunSystemCmd(NULL_FILE, "touch", "/var/BridgeToRout", NULL_STR);
...
//		}
	...
	/*Zyxel bridge setup*/
	RunSystemCmd(NULL_FILE, "/etc/init.d/zyxel_bridgeSetup", "boot", NULL_STR);
