...
char wlan_virtual_interface[120]={0};
...
void set_br_interface(unsigned char *brif)
{
  ...
  apmib_get(MIB_OP_MODE,(void *)&opmode);
	...
	if(num_wlan_mesh_interface > 0) {        
        int i;
    #ifdef CONFIG_RTL_TRI_BAND
        int wlan_disable, orig_wlan_idx = wlan_idx;
    #endif
        for (i=0; i<num_wlan_mesh_interface; i++) {
    #ifdef CONFIG_RTL_TRI_BAND
            wlan_idx = i;
            ...
            if (wlan_disable == 1)
                continue;
    #endif
            memset(wlan_mesh_interface, 0, sizeof(wlan_mesh_interface));
    		sprintf(wlan_mesh_interface, "wlan%d-msh0", i);
    		strcat(tmpBuff, wlan_mesh_interface);
    		strcat(tmpBuff, " ");
        }
    #ifdef CONFIG_RTL_TRI_BAND
...
#if 0	// ZYXEL - don't kill dnsmasq process here
...
static void rtl_configRps(void)
{
	...
	system("echo 1 > /proc/irq/39/smp_affinity");
	system("echo 2 > /proc/irq/40/smp_affinity");
	system("echo 2 > /proc/irq/42/smp_affinity");
  #endif
	system("echo 1 > /sys/class/net/wlan0/queues/rx-0/rps_cpus");
	system("echo 2 > /sys/class/net/wlan1/queues/rx-0/rps_cpus");
	system("echo 1 > /sys/class/net/wlan2/queues/rx-0/rps_cpus");
#endif /* CONFIG_RTL_TRI_BAND */
}
#endif
...
/*currently, we just support init gw/ap all */	
	...
	{
		/*They will support ZYXEL solution, so add them.*/
		if ( NULL != pf && opmode == GATEWAY_MODE ){
			FILE *pf2=NULL;
			pf2=fopen("/var/BridgeToRout","r");
			if ( NULL != pf2 ){
				clean_process(opmode,wan_dhcp_mode,gateway, enable_wan, br_interface, wlan_interface, wan_interface);
				RunSystemCmd(NULL_FILE, "/etc/init.d/network", "start", NULL_STR);
				fclose(pf2);
				RunSystemCmd(NULL_FILE, "rm", "/var/BridgeToRout", NULL_STR);
				RunSystemCmd(NULL_FILE, "rm", "/var/Bridge_trigger_port", NULL_STR);
			}
			printf("============== Support ZYXEL solution. ============== \n");
		} else {
			clean_process(opmode,wan_dhcp_mode,gateway, enable_wan, br_interface, wlan_interface, wan_interface);
		}
#if !defined(CONFIG_RTL_AP_PACKAGE) 
...
//printf("\r\n wan_interface=[%s],__[%s-%u]\r\n",wan_interface,__FILE__,__LINE__);				
			...
			//AP mode from Zyxel_bridgeSetup set WAN mtu 
			// if(wan_interface[0])
...
#if defined (CONFIG_APP_VSFTPD)
#if defined (CONFIG_APP_VSFTPD_NO_START_BY_SYSCONF)
#else
	start_vsftpd();
#endif
#endif	
...
//			RunSystemCmd(NULL_FILE, "rm", "-f", "/web/*.cpudat", NULL_STR);
	...
	if(!reinit)
	{
#ifndef CONFIG_APP_ZAPI_BLE_SERVICE
		...
		system("mount -t sysfs sysfs /sys");
		system("echo 1 > /sys/class/firmware/timeout");
#endif /* CONFIG_APP_ZAPI_BLE_SERVICE */
