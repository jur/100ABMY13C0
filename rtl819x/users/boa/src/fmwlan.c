...
int addWlProfileHandler(request *wp, char *tmpBuf, int wlan_id)
{
	...
	if(add_to_profile == 1)
	{
		...
		else if(wlan_id == 2)
		{
			...
			profile_tbl_id = MIB_PROFILE_TBL3;
			profile_add_id = MIB_PROFILE_ADD3;
			profile_delall_id = MIB_PROFILE_DELALL3;			
		}
#endif
...
//displayPostDate(wp->post_data);
	...
	else if(wlan_id == 2)
	{
		...
		profile_tbl_id = MIB_PROFILE_TBL3;
		profile_add_id = MIB_PROFILE_ADD3;		
		profile_del_id = MIB_PROFILE_DEL3;
		profile_delall_id = MIB_PROFILE_DELALL3;
...
#if defined(CONFIG_RTL_TRI_BAND)
		apmib_set(MIB_REPEATER_ENABLED3, (void *)&val);	
#endif	
    }
    else
#endif
...
//displayPostDate(wp->post_data);
	...
	else if(wlan_idx == 2)
	{
		profile_num_id = MIB_PROFILE_NUM3;
		profile_tbl_id = MIB_PROFILE_TBL3;
		profile_mod_id = MIB_PROFILE_MOD3;
	}
#endif
...
void formWlanMultipleAP(request *wp, char *path, char *query)
{
    ...
    if ( !strcmp(strVal, "ON")){
        ...
        if(wlan_idx == 0) {
			...
			apmib_set(MIB_PROFILE_ENABLED3, (void *)&valTmp);
		}
#endif
    ...
    if(wlan_idx==0){
        ...
        apmib_set(MIB_REPEATER_ENABLED3, (void *)&repeater_enabled);
        SDEBUG("MIB_REPEATER_ENABLED3 mode set to [%d]\n",repeater_enabled);
    } 
#endif           
#endif        
